...
r.squaredGLMM(model.oc.pres.rac)

rsq(model.oc.pres.rac)
rsq.partial(model.oc.pres.rac, adj=FALSE)
...
pr2_pres_ntz_df <- data.frame(
  Variable = names(pr2.oc.pres.ntz),         # Variable names from the list
  Partial_R2 = unlist(pr2.oc.pres.ntz))    # Partial R^2 values from the list


pres.vrsq.ntz<- cbind(pr2_pres_ntz_df$Variable, pr2_pres_ntz_df$Partial_R2)
pres.vrsq.ntz.zeros<- data.frame(V1 = c("dist", "precipitation","elev_range", "urbanland","temperature"),  # create vector for unsignificant variables with zero
                            V2 = c(0,0,0,0,0))
...
#M4 (native species)
pr2_pres_ntv_df <- data.frame(
  Variable = names(pr2.oc.pres.ntv),         # Variable names from the list
  Partial_R2 = unlist(pr2.oc.pres.ntv)       # Partial R^2 values from the list
)

pres.vrsq.ntv<- cbind(pr2_pres_ntv_df$Variable, pr2_pres_ntv_df$Partial_R2)
pres.vrsq.ntv.zeros<- data.frame(V1 = c("abs.lat","area:dist","elev_range", "urbanland"),
                            V2 = c(0,0,0,0))
...
#Saving the plot as a png
png("figures/newM3M5_var_imp_oc_pres.jpg", width=10, height= 10, units='in', res=300)
combined.plot.oc.pres
dev.off()


####OLDVERSION BEFORE 23.10
#M3 (naturalized species)
...
#combined.plot.oc.pres<- ggplot(pres.comb.var.impo3, aes(x=variable, y=partialrsq, fill=status, color = status)) +
#  geom_bar(stat="identity", position="dodge", alpha=0.6) +
#  colScale+  fillScale+  theme_minimal()+
#  theme(axis.text.x = element_text(angle = 90 ,size=20), axis.text.y = element_text(angle = 90, size=20))+
  #coord_flip()+
...
#naturalized
pr2_prop_ntz_df <- data.frame(
  Variable = names(pr2.oc.prop.ntz),         # Variable names from the list
  Partial_R2 = unlist(pr2.oc.prop.ntz))    # Partial R^2 values from the list


vrsq.ntz<- cbind(pr2_prop_ntz_df$Variable, pr2_prop_ntz_df$Partial_R2) #create dataframe for plot
vrsq.ntz.zeros<- data.frame(V1 = c("precipitation","temperature","area:dist","urbanland","elev_range"),
                            V2 = c(0,0,0,0,0))
...
#native
pr2_prop_ntv_df <- data.frame(
  Variable = names(pr2.oc.prop.ntv),         # Variable names from the list
  Partial_R2 = unlist(pr2.oc.prop.ntv))    # Partial R^2 values from the list

vrsq.ntv<- cbind(pr2_prop_ntv_df$Variable, pr2_prop_ntv_df$Partial_R2)
vrsq.ntv.zeros<- data.frame(V1 = c("urbanland","elev_range"),
                            V2 = c(0,0))
...
#Saving the plot as a png
png("figures/newM4M6_var_imp_oc_prop.jpg", width=10, height= 10, units='in', res=300)
combined.plot.oc.prop
dev.off()
