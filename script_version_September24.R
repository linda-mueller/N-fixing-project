#N-fixing project: Script version September 2024
...
####BROAD PRESENCE MODEL####

#mainland dataset for broad presence model
gdat.ml.pres <- gdat.ref %>%
  ...
  filter(!landtype=="other_island")%>%
  mutate(area = as.vector(log10((area)+.01)))%>%
  drop_na()%>%
  mutate_at(c("abs.lat","area","elev_range","precipitation", "temperature"), scale)
...
ref.table.land.stat.rac.ntz <- ref.table.land.stat.rac%>%filter(!status=="native") #only naturalized 
ref.table.land.stat.rac.ntv <- ref.table.land.stat.rac%>%filter(!status=="naturalized")
...
#gdat3cat<- gdat.ml %>% mutate(proppresence=(presence==1)/(presence==0))
...
broad.pres.ntz<- 
  ...
  xlab(" ")+
  #ggtitle("Model presence~ Landtype* Status")+
  ...
  theme(axis.text.x=element_text(size=50))+      # All font sizes
  ylim(0,1)
...
broad.pres.ntv<- 
  ...
  theme(axis.title.y=element_text(size=50))# All font sizes
...
####BROAD PROPORTION MODEL####
gdat.ml.prop <- gdat.ref %>%
  dplyr::select(c("entity_ID","nfix","nfixno", "latitude","abs.lat","longitude", # Select columns
                  "landtype","status","presence","area","elev_range","precipitation", "temperature"))%>%
  filter(!landtype=="other_island")%>%
  mutate(area = as.vector(log10((area)+.01)))%>%
  #!!! find out values that have no species
...
#### Model Proportion ~ Land type *status +abs.lat +area +elev_range +precipitation
...
gdat.isl.ntz <- gdat.ref %>%
  ...
  #filter(area>6)%>%
  add_row(entity_ID = 0000, entity_class="Island", area=6)%>% # breakpoint for testing of small island effect
...
####MODEL naturalized oceanic presence####
#presence~abs.lat +area +dist +elev_range	+precipitation+temperature +urbanland	+area:dist +urbanland:dist, 285 objects
...
cont.var <- c("abs.lat", "elev_range","area","dist","precipitation")
Mypairs(dat[,cont.var]) # area and elevation range 0.72
MyMultipanel.ggp2(dat, cont.var, vary = "presence", ylab = "presence") # pretty hard to see what's going on
...
E2 <- resid(model.oc.prop.rac, type = "pearson")
#list of variables: abs.lat, area, dist, elev_range, precipitation
plot(E2 ~ precipitation, data = oceanic.prop.ntv)
#outlier in precipitation
...
#Saving the plot as a png
png("figures/var_imp_oc_prop_combinedsc.jpg", width=10, height= 10, units='in', res=300)
combined.plot.oc.prop
dev.off()
